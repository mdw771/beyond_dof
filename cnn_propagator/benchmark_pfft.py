...
for this_size in np.take(size_ls, range(i_starting_size, len(size_ls))):
    ...
    for n_slices in np.take(n_slices_ls, range(i_starting_nslice, len(n_slices_ls))):
        ...
        for i in range(n_repeats):
            ...
            t0 = time.time()
            if rank != 0:
                print(wavefield.real)
                comm.send(wavefield.real, dest=0)
            ...
            if rank == 0:
                ...
                for i_src_rank in range(1, n_ranks):
                    ...
                    try:
                        this_block_real = comm.recv(source=i_src_rank)
            ...
            if rank != 0:
                comm.send(wavefield.imag, dest=0)
                # comm.Send(wavefield, dest=0, tag=1)
            if rank == 0:
                full_wavefield = np.zeros([n_batch, *original_grid_shape[:-1]], dtype=np.complex)
                ...
                for i_src_rank in range(1, n_ranks):
                    ...
                    # this_block = np.zeros([n_src_rank_stack, *wavefield.shape[1:]])
                    try:
                        this_block_imag = comm.recv(source=i_src_rank)
                        # comm.Recv(this_block, source=i_src_rank, tag=1)
                    except:
                        pass
