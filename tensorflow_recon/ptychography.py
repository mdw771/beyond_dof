...
def reconstruct_ptychography(fname, probe_pos, probe_size, obj_size, theta_st=0, theta_end=PI, theta_downsample=None,
    ...
    def rotate_and_project(i, obj):
        ...
        for k, pos_batch in enumerate(probe_pos_batch_ls):
            ...
            if forward_algorithm == 'fresnel':
                ...
                exiting = multislice_propagate_fd(subobj_ls[:, :, :, :, 0], subobj_ls[:, :, :, :, 1], probe_real,
                                                  probe_imag,
                                                  energy_ev, psize_cm * ds_level, h=h, free_prop_cm='inf',
                                                  obj_batch_shape=[len_subobj, *probe_size, obj_size[-1]])
    ...
    if core_parallelization is False:
        ...
        try:
            ...
            warnings.warn(
                'Unable to import mpi4py. Using multiple threads with n_epoch set to "auto" may lead to undefined behaviors.')
    ...
    for ds_level in range(multiscale_level - 1, -1, -1):
        ...
        if initializer_flag == False:
            if initial_guess is None:
                ...
                obj_init[:, :, :, 0] = np.random.normal(size=obj_size, loc=grid_delta.mean(),
                                                        scale=grid_delta.mean() * 0.05)
                obj_init[:, :, :, 1] = np.random.normal(size=obj_size, loc=grid_beta.mean(),
                                                        scale=grid_beta.mean() * 0.05)
        ...
        if probe_type == 'gaussian':
            ...
            if probe_initial is not None:
                ...
                back_prop_cm = (free_prop_cm + (psize_cm * obj_init.shape[2])) if free_prop_cm is not None else (
                            psize_cm * obj_init.shape[2])
        ...
        if alpha_d is None:
            ...
            if gamma == 0:
                ...
                reg_term = alpha_d * tf.norm(obj[:, :, :, 0], ord=1) + alpha_b * tf.norm(obj[:, :, :, 1],
                                                                                         ord=1) + gamma * total_variation_3d(
                    obj[:, :, :, 0])
        ...
        if probe_type == 'optimizable':
            ...
            if n_batch_per_update > 1:
                ...
                initialize_grad_probe = [accum_grad_probe[i].assign(tf.zeros_like(accum_grad_probe[i])) for i in
                                         range(2)]
                accum_op_probe = [accum_grad_probe[i].assign_add(this_grad_probe[i][0]) for i in range(2)]
                update_probe = [
                    optimizer_probe.apply_gradients([(accum_grad_probe[i] / n_batch_per_update, this_grad_probe[i][1])])
                    for i in range(2)]
            else:
        ...
        for epoch in range(n_loop):
            ...
            try:
                if minibatch_size < n_theta:
                    ...
                    for i_batch in range(n_batch):
                        ...
                        this_prj_numpy = prj[this_i_theta, ind_list_rand[i_batch][hvd.rank() * minibatch_size:(
                                                                                                                          hvd.rank() + 1) * minibatch_size,
                                                           1]]
                        # print(ind_list_rand[i_batch][hvd.rank() * minibatch_size:(hvd.rank() + 1) * minibatch_size, 1])
                        this_pos_batch = probe_pos[
                            ind_list_rand[i_batch][hvd.rank() * minibatch_size:(hvd.rank() + 1) * minibatch_size, 1]]
                        if ds_level > 1:
                        ...
                        if n_batch_per_update > 1:
                            ...
                            if probe_type == 'optimizable':
                                ...
                                _, current_loss, current_reg, summary_str = sess.run(
                                    ...
                                    feed_dict={theta_placeholder: this_theta_numpy,
                                               prj_placeholder: this_prj_numpy,
                                               pos_placeholder: this_pos_batch})
                            print_flush('Minibatch done in {} s (rank {}); current loss = {}, probe reg. = {}.'.format(
                            ...
                            if probe_type == 'optimizable':
                                _, _, current_loss, current_reg, current_probe_reg, summary_str = sess.run(
                                    [optimizer, optimizer_probe, loss, reg_term, probe_reg, merged_summary_op],
                                    options=run_options, run_metadata=run_metadata,
                                    feed_dict={theta_placeholder: this_theta_numpy,
                                               prj_placeholder: this_prj_numpy,
                                               pos_placeholder: this_pos_batch})
                                ...
                                ###
                                _, current_loss, current_reg, summary_str = sess.run(
                                    [optimizer, loss, reg_term, merged_summary_op],
                                    options=run_options, run_metadata=run_metadata,
                                    feed_dict={theta_placeholder: this_theta_numpy,
                                               prj_placeholder: this_prj_numpy,
                                               pos_placeholder: this_pos_batch})
                    ...
                    if probe_type == 'optimizable':
                        _, _, current_loss, current_reg, summary_str = sess.run(
                            [optimizer, optimizer_probe, loss, reg_term, merged_summary_op],
                            options=run_options, run_metadata=run_metadata,
                            feed_dict={theta_placeholder: this_theta_numpy,
                                       prj_placeholder: this_prj_numpy,
                                       pos_placeholder: this_pos_batch})
                ...
                warnings.warn('Rank {} did not conduct optimization operation. This might be because the total'
                              'number of spots is undividable among all ranks and it ran into an empty batch,'
                              'but it is also possible that something went wrong.'.format(hvd.rank()))
            ...
            if n_epochs == 'auto':
                if len(loss_ls) > 0:
                    ...
                    sys.stdout.flush()
                if len(loss_ls) > 0 and -crit_conv_rate < (current_loss - loss_ls[-1]) / loss_ls[
                    -1] < 0 and hvd.rank() == 0:
            ...
            if save_intermediate and hvd.rank() == 0:
                ...
                if full_intermediate:
                    dxchange.write_tiff(temp_obj[:, :, :, 0],
                                        fname=os.path.join(output_folder, 'intermediate',
                                                           'ds_{}_iter_{:03d}'.format(ds_level, epoch)),
                                        dtype='float32',
                                        overwrite=True)
                else:
                    ...
                    probe_current_mag, probe_current_phase = real_imag_to_mag_phase(probe_current_real,
                                                                                    probe_current_imag)
                    ...
                    dxchange.write_tiff(probe_current_phase,
                                        ...
                                        dtype='float32',
                                        overwrite=True)
                dxchange.write_tiff(temp_obj[:, :, :, 0], os.path.join(output_folder, 'current', 'delta'),
                                    dtype='float32', overwrite=True)
                dxchange.write_tiff(temp_obj[:, :, :, 1], os.path.join(output_folder, 'current', 'beta'),
                                    dtype='float32', overwrite=True)
                print_flush('Iteration {} (rank {}); loss = {}; time = {} s'.format(epoch, hvd.rank(), current_loss,
                                                                                    time.time() - t00))
            sys.stdout.flush()
            # except:
        ...
        if hvd.rank() == 0:
            ...
            res = sess.run(obj)
            dxchange.write_tiff(res[:, :, :, 0], fname=os.path.join(output_folder, 'delta_ds_{}'.format(ds_level)),
                                dtype='float32', overwrite=True)
            dxchange.write_tiff(res[:, :, :, 1], fname=os.path.join(output_folder, 'beta_ds_{}'.format(ds_level)),
                                dtype='float32', overwrite=True)
            ...
            dxchange.write_tiff(probe_final_mag, fname=os.path.join(output_folder, 'probe_mag_ds_{}'.format(ds_level)),
                                dtype='float32', overwrite=True)
            dxchange.write_tiff(probe_final_phase,
