...
for i in range(n_repeats):
    ...
    if block_delta_batch.shape[0] > 0:
        comm.Barrier()
        # -----------------------------------------
        block_delta_batch = w.create_variable(block_delta_batch, device=device_obj)
        block_beta_batch = w.create_variable(block_beta_batch, device=device_obj)
        block = w.stack([block_delta_batch, block_beta_batch], -1)
        block_probe_real_batch = w.create_variable(block_probe_real_batch, device=device_obj)
        block_probe_imag_batch = w.create_variable(block_probe_imag_batch, device=device_obj)
