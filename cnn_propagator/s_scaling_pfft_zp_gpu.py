...
import adorym.global_settings as ag
from adorym import print_flush

ag.backend = 'pytorch'
device_obj = w.get_device(0)
...
if rank == 0:
    ...
    if os.path.getsize(os.path.join(path_prefix, 'size_{}'.format(this_size), 'nd_{}'.format(n_nodes), 'report_pfft.csv')) == 0:
        f.write('i_repeat,n_nodes,this_size,n_slices,safe_zone_width,n_blocks_y,n_blocks_x,block_size,n_ranks,dt_total,dt_read_div,dt_reshape_and_comm,dt_fft_prop\n')
...
for i in range(n_repeats):
    ...
    if rank == 0:
        print_flush('n_blocks_y: ', n_blocks_y)
        print_flush('n_blocks_x: ', n_blocks_x)
        print_flush('n_blocks: ', n_blocks)
        print_flush('block_size: ', block_size)
    ...
    if block_delta_batch.shape[0] > 0:
        comm.Barrier()
        # -----------------------------------------
        t_reshape_0 = time.time()
        block_delta_batch = w.create_variable(block_delta_batch, device=device_obj, requires_grad=False)
        block_beta_batch = w.create_variable(block_beta_batch, device=device_obj, requires_grad=False)
        block = w.stack([block_delta_batch, block_beta_batch], -1)
        block_probe_real_batch = w.create_variable(block_probe_real_batch, device=device_obj, requires_grad=False)
        block_probe_imag_batch = w.create_variable(block_probe_imag_batch, device=device_obj, requires_grad=False)
        dt_reshape = time.time() - t_reshape_0
        wave_real, wave_imag, dt = adorym.multislice_propagate_batch(block,
                                                         block_probe_real_batch, block_probe_imag_batch, energy_ev,
                                                         psize_cm, delta_cm=slice_spacing_cm,
                                                         obj_batch_shape=block_delta_batch.shape,
                                                         return_fft_time=True,
                                                         repeating_slice=n_slices, device=device_obj)
        comm.Barrier()
        dt_fft_prop = dt
        t_reshape_0 = time.time()
        wave_real = w.to_numpy(wave_real)
        wave_imag = w.to_numpy(wave_imag)
        wavefield = wave_real + 1j * wave_imag
        dt_reshape = time.time() - t_reshape_0 + dt_reshape
        # -----------------------------------------
        t_write_0 = time.time()
#        comm.Barrier()
...
#        comm.Barrier()
    ...
    if rank == 0:
        print_flush('PFFT: For size {}, average dt = {} s.'.format(this_size, dt_total))
        f.write('{},{},{},{},{},{},{},{},{},{},{},{},{}\n'.format(i, n_nodes, this_size, n_slices, safe_zone_width, n_blocks_y, n_blocks_x, block_size, n_ranks, dt_total, dt_read_div, dt_reshape, dt_fft_prop))
        f.flush()
        os.fsync(f.fileno())
