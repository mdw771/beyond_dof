...
for this_size in size_ls:
    ...
    grid_delta = np.swapaxes(np.swapaxes(grid_delta, 1, 2), 0, 1)
    grid_beta = np.swapaxes(np.swapaxes(grid_beta, 1, 2), 0, 1)
...
for this_size in size_ls:
    ...
    print(block_size, n_blocks_y, n_blocks_x)
    ...
    for i_pos in this_pos_ind_ls:
        ...
        sub_probe_real = np.pad(probe_real, [[pad_top, pad_bottom], [pad_left, pad_right]], mode='edge')
        sub_probe_imag = np.pad(probe_imag, [[pad_top, pad_bottom], [pad_left, pad_right]], mode='edge')

        print(sub_grid_delta.shape, block_delta_batch.shape)

        block_delta_batch[i_pos, :, :, :] = sub_grid_delta
        block_beta_batch[i_pos, :, :, :] = sub_grid_beta
        block_probe_real_batch[i_pos, :, :] = sub_probe_real
        block_probe_imag_batch[i_pos, :, :] = sub_probe_imag
    ...
    for i in range(n_repeats):
        ...
        wavefield, dt = multislice_propagate_batch_numpy(block_delta_batch, block_beta_batch, block_probe_real_batch, block_probe_imag_batch, energy_ev,
                                                         [psize_cm] * 3, obj_batch_shape=grid_delta.shape,
        ...
        t0 = time.time()
        this_full_wavefield = np.zeros([n_batch, *original_grid_shape[:-1]], dtype='complex64')
        this_full_wavefield[0, line_st:line_end, px_st:px_end] = wavefield[:,
                                                                 safe_zone_width:safe_zone_width + (line_end - line_st),
